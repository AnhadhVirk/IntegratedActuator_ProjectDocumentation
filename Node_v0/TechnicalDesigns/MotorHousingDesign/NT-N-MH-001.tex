...
\subsubsection{Output Force Analysis}
According to NT-N-IRQ1, the actuator should be able to apply 200Nm at a lever arm of 0.25m, up to 50Hz.
Resisting axial forces as a result of moving loads is not specified in the requirements and is therefore not included in the load model.
...
The force and load assessment of the flange is used to select the correct output transmission bearing.
\\\\
...
In the simplified version of the free body diagram, if the beam is subject to the entire weight of the load (which is at a maximum 196N derived from NT-N-IRQ1) in the axial direction, then the output bearing must resist axial loads of up to 196N.
\\\\
...
\\\\
The output of the cycloidal disc is transmitted through bushings on the output rollers, which must additionally carry radial forces ontop of the useful torque generation.
These radial forces are dependent on the output torque, number of output rollers and diameter of the output rollers (including the bushings on top).
From Li et al's work in \cite{OutputRollerBearingAnalysis}, the following equilibrium describes the radial force of the bearings:

The bearing requirement summary is shown in Table~\ref{tab:bearing-requirements}.
\begin{table}[H]
  \centering
  \caption{Bearing requirement summary}
  \label{tab:bearing-requirements}
  \begin{tabular}{|l|l|l|}
    \hline
    Requirement & Value & Basis \\
    \hline
    Axial load capacity & 196N & Maximum payload from NT-N-IRQ1 \\
    Radial load capacity & 196N & Equivalent radial reaction from payload \\
    Overturning moment capacity & 48Nm & $M = P \cdot y$ with $P = 196N$, $y = 0.25m$ \\
    Operating speed & 600RPM & 20Hz at half-revolution motion \\
    Service life & 10,000 hours & NT-N-IRQ9 \\
    Environmental protection & Sealed bearing & NT-N-IRQ10 \\
    Factor of safety & 2 & NT-N-IRQ13 \\
    \hline
  \end{tabular}
\end{table}

\subsubsection{Output Bearing Selection}
Since the output interface is a flange, and considering all other requirements for the bearing, a crossed roller bearing with outer and inner mounting holes is the most suitable architecture for the output bearing.


\subsubsection{Cycloidal Reducer Design}
Along with this, when the difference between the number of rollers and lobes is greater than 1, less than half of the lobes stay engaged during eccentric motion, even in the ideal case (\cite{CycloidalReducerLoadAnalysis}).
With a difference of 1, every lobe is engaged (in ideal conditions) and the load on each individual lobe is drastically reduced.
Therefore this will be the architecture used in this implementation.
...
\begin{thebibliography}{9}
  ...
  \bibitem{CycloidalReducerLoadAnalysis}
  Sensinger, J. W. (February 9, 2010). "Unified Approach to Cycloid Drive Profile, Stress, and Efficiency Optimization." ASME. J. Mech. Des. February 2010; 132(2): 024503. https://doi.org/10.1115/1.4000832

  \bibitem{OutputRollerBearingAnalysis}
  Li, X., Li, C., Wang, Y., Chen, B., and Lim, T. C. (January 16, 2017). "Analysis of a Cycloid Speed Reducer Considering Tooth Profile Modification and Clearance-Fit Output Mechanism." ASME. J. Mech. Des. March 2017; 139(3): 033303. https://doi.org/10.1115/1.4035541
